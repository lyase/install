 ...
 * formattedHTML
 */
const WString ChatEvent::formattedHTML(const WString& user, TextFormat format) const
{
    switch (type_)
    {
        case Login:
            return WString::fromUTF8("<span class='chat-info'>") + WWebWidget::escapeText(user_) + " joined.</span>";
        case Logout:
            return WString::fromUTF8("<span class='chat-info'>") + ((user == user_) ? WString::fromUTF8("You") : WWebWidget::escapeText(user_)) + " logged out.</span>";
        case Rename:
            return "<span class='chat-info'>" + ((user == data_ || user == user_) ? "You are" : (WWebWidget::escapeText(user_) + " is")) + " now known as " + WWebWidget::escapeText(data_) + ".</span>";
        case Message:
            {
                WString result;

                result = WString("<span class='") + ((user == user_) ? "chat-self" : "chat-user") + "'>" + WWebWidget::escapeText(user_) + ":</span>";

                WString msg = (format == XHTMLText ? message_ : WWebWidget::escapeText(message_));
 ...
 * SimpleChatServer
 */
SimpleChatServer::SimpleChatServer(WServer& server) : server_(server)
{
 ...
 * connect
 */
bool SimpleChatServer::connect(Client *client, const ChatEventCallback& handleEvent)
{
...
void SimpleChatServer::postChatEvent(const ChatEvent& event)
{
     ...
     */
        ...
        {
            server_.post(i->second.sessionId, boost::bind(i->second.eventCallback, event));
        }
    }
}
/* ************************************************************************* */
