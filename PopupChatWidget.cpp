 ...
 * PopupChatWidget
 */
PopupChatWidget::PopupChatWidget(SimpleChatServer& server, const std::string& id) : SimpleChatWidget(server), missedMessages_(0)
{
    setId(id);
    ...
    implementJavaScript(&PopupChatWidget::toggleSize, "{""var s = $('#" + id + "');""s.toggleClass('chat-maximized chat-minimized');" + Wt::WApplication::instance()->javaScriptClass() + ".layouts2.scheduleAdjust(true);}");
 ...
 * createLayout
 */
void PopupChatWidget::createLayout(Wt::WWidget *messages, Wt::WWidget *userList, Wt::WWidget *messageEdit, Wt::WWidget *sendButton, Wt::WWidget *logoutButton)
{
    (void)userList; // Eat userList warning
    (void)sendButton; // Eat sendButton warning
    (void)logoutButton; // Eat logoutButton warning
...
void PopupChatWidget::updateUsers()
{
    ...
    {
        title_->setText(Wt::WString("Chat: <span class=\"self\">{1}</span> <span class=\"online\">({2} user{3})</span>").arg(userName()).arg(count).arg(count == 1 ? "" : "s"));
    }
}
/* ************************************************************************* */
/*
